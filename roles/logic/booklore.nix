...
}:
{
  ...
  config = lib.mkIf config.lab.booklore.enable {
    ...
    virtualisation.oci-containers.containers."booklore" = rec {
      podman = {
        user = "booklore";
      };
      ...
      pull = "newer";
      extraOptions = [
        "--network=pasta:--tcp-ports,${environment.BOOKLORE_PORT},--tcp-ns,${toString config.services.mysql.settings.mysqld.port}"
